...
if (!vehicleId) {
  container.innerHTML = '<div class="empty-state">No se encontro el vehiculo solicitado.</div>';
}
  ...
  return `
    <div class="gallery">
      <div class="gallery-main">
        <img id="mainVehicleImage" src="${window.TicoAutos.buildImageUrl(validImages[0])}" alt="Imagen principal del vehiculo" />
      </div>
          ...
          .map(
            (image, index) => `
              <button class="gallery-thumb" type="button" data-gallery-image="${index}">
                <img src="${window.TicoAutos.buildImageUrl(image)}" alt="Miniatura ${index + 1}" />
              </button>
            `
          )
          .join("")}
...
const loadVehicleDetail = async () => {
  try {
    ...
    container.innerHTML = `
      <div class="detail-layout">
        ...
        <aside class="detail-panel">
          ...
          <p class="page-subtitle">${window.TicoAutos.escapeHtml(vehicle.description || "Publicacion sin descripcion detallada.")}</p>
          ...
          <div class="detail-owner">
            <h2>Propietario</h2>
            <p class="muted">${window.TicoAutos.escapeHtml(owner?.name || "")} ${window.TicoAutos.escapeHtml(owner?.lastname || "")}</p>
    ...
    console.error(error);
    container.innerHTML =
      '<div class="empty-state">No se pudo cargar el detalle del vehiculo. Revisa el ID o el backend.</div>';
  }
};
